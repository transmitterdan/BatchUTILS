...
@echo cmake -Wno-dev -G "%vcgen%" -T v%vcts% -D CMAKE_CXX_FLAGS="/D_USING_V110_SDK71_ /MP /EHsc" -D CMAKE_C_FLAGS="/MP /D_USING_V110_SDK71_" CMAKE_EXE_LINKER_FLAGS=/SUBSYSTEM:WINDOWS%XP_FLAG% CMAKE_MODULE_LINKER_FLAGS=/SUBSYSTEM:WINDOWS%XP_FLAG% CMAKE_SHARED_MODULE_LINKER_FLAGS=/SUBSYSTEM:WINDOWS%XP_FLAG% ..
